...
void loop() {
  ...
  if (state == MEASURING_ALC) {
    if (millis() - startTime < 5000) {
      ...
      float RS_gas = ((5.0 * 2000) / sensorVolt) - 2000;
      float ratio = RS_gas / R0;
      double x = 0.4 * ratio;
      float BAC = pow(x, -1.431);
      if (BAC > maxBAC) {
        maxBAC = BAC;
      }
      delay(100);
  ...
  if (state == MEASURING_TEMP) {
    if (millis() - startTime < 5000) {
      ...
      lcd.print(5 - remaining);
      lcd.print("s");
      // here will go code for temp measuring now it will only return 36.6
      float temp = 36.6;
      if (temp > maxTemp) {
        maxTemp = temp;
      }
  ...
  if (state == EVALUATION) {
    ...
    unsigned long start_timer = micros();
    bool report = radio.write(&canAttend, sizeof(canAttend));
    unsigned long end_timer = micros();
