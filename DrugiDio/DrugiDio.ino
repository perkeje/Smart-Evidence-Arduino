...
void setup() {
  ...
  //   connected = attendanceSocket.connect("ws://raspberrypi.local:8888/evidence_ws");
  //   delay(500);
  // }
  bool connected = false;
  while (!connected) {
    Serial.println("WS connecting");
    connected = rfidSocket.connect("ws://raspberrypi.local:8888/rfid_ws");
    delay(500);
  }
...
void loop() {
  ...
  if (state == AUTHENTICATING) {
    ...
    if (report) {
      ...
      Serial.print(F("Time to transmit = "));        // print the timer result
      Serial.print(F(" us. Sent: "));
      Serial.println(String((char*)name));  // print payload sent
      radio.startListening();
      radio.setPayloadSize(sizeof(canAttend));
      memset(name, NULL, 32);
      memset(uuid, NULL, 10);
      state = EVALUATING;
    } else {
